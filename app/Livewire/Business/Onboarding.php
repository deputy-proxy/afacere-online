...
final class Onboarding extends Component
{
    ...
    {
        if ($businessContext->current($this->user()) !== null) { $this->redirectRoute('dashboard'); }
    }
    ...
    public function createBusiness(BusinessContextService $businessContext, SubscriptionService $subscriptions): void
    {
        ...
        $businessContext->create($this->user(), $validated['name'], $validated['description'] !== '' ? $validated['description'] : null);
        $this->redirectRoute('dashboard');
    }

    private function user(): User { $user = Auth::user(); abort_unless($user instanceof User, 401); return $user; }
    public function render(): mixed { return view('livewire.business.onboarding'); }
}
